...
await writeStaticPage('/', {
  ...
  structuredData: [
    ...
    },
    {
      ...
      potentialAction: {
        '@type': 'SearchAction',
        target: `${SITE.origin}/resources/deepfake-detection-online?q={search_term_string}`,
    ...
    },
    {
      ...
      mainEntity: [
        {
          '@type': 'Question',
          name: 'What does the Deepfake Detection API return?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'It returns a probability score, confidence, anomaly regions, video timeline values, audio evidence, and webhook-ready metadata.',
          },
        },
        {
          '@type': 'Question',
          name: 'Which plan is selected by default?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'The Pro annual plan is selected by default, with annual billing priced 50% lower than monthly billing.',
          },
        },
      ],
    },
...
await writeStaticPage('/privacy', {
  ...
  description: 'How Deepfake Detection handles demo metadata, checkout, support, analytics, and API-related communications.',
  robots: 'index,follow',
  canonicalPath: '/privacy',
  rootHtml: buildLegalPrerender(
    'Privacy Policy',
    'Deepfake Detection collects only the information needed to operate the site, demo scanner, checkout, support, analytics, and API communications.',
  ),
  structuredData: [],
})
...
await writeStaticPage('/terms', {
  ...
  description: 'Terms for using Deepfake Detection scans, API materials, checkout, support, and related documentation.',
  robots: 'index,follow',
  canonicalPath: '/terms',
  rootHtml: buildLegalPrerender(
    'Terms of Service',
    'Deepfake Detection provides probabilistic screening and workflow routing. It does not guarantee fraud, legal, audit, or litigation outcomes.',
  ),
  structuredData: [],
})
...
function renderHtml({ title, description, robots, canonicalPath, rootHtml, structuredData }) {
  ...
  if (graph) {
    html = html.replace(
      '</head>',
      `    <script type="application/ld+json" id="deepfake-prerender-schema">${JSON.stringify(graph)}</script>\n  </head>`,
    )
  }
  ...
  return `
    <main class="article-wrap">
      <article class="article">
        <a class="back-link" href="/">Deepfake Detection API</a>
